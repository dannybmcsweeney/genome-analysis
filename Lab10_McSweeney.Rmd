...
daily_report <-   read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/04-02-2020.csv")) %>% 
  ...
  mutate(Admin2 = tolower(Admin2))
us <- map_data("state")
ga_us <- subset(us, region == "georgia")
counties <- map_data("county")
ga_county <- subset(counties, region == "georgia")
state_join <- left_join(ga_county, daily_report, by = c("subregion" = "Admin2")) 
# plot state map
ggplot(data = ga_county, mapping = aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) + 
# Add data layer
  geom_polygon(data = state_join, aes(fill = Confirmed), color = "white") +
    scale_fill_gradientn(colors = brewer.pal(n = 5, name = "BuGn"),
                         trans = "log10") +
  labs(title = "COVID-19 Confirmed Cases in Georgia'")

...
```{r}
##Graph No. 1: Graph of number of confirmed cases for states found in Canada.
report_03_11_2020 %>% 
  filter (Country.Region == "Canada") %>% 
  ggplot(aes(x = Confirmed,  y = reorder(Province.State, Confirmed))) + 
    geom_point() +
    ggtitle("Confirmed cases for each Canada province") +
    ylab("Country/Region") +
    xlab("Confirmed Cases")
```

```{r}
##Graph No. 2: Graph of number of confirmed deaths for states found in Canada.
report_03_11_2020 %>% 
  filter (Country.Region == "Canada") %>% 
  ggplot(aes(x = Deaths,  y = reorder(Province.State, Deaths))) + 
    geom_point() +
    ggtitle("Confirmed deaths for each Canada province") +
    ylab("Country/Region") +
    xlab("Confirmed Deaths")
```

```{r}
##Graph No. 2: Graph of number of confirmed casess for states found in China
report_03_11_2020 %>% 
  filter (Country.Region == "China") %>% 
  ggplot(aes(x = Confirmed,  y = reorder(Province.State, Confirmed))) + 
    geom_point() +
    ggtitle("Confirmed cases for each Chinese State") +
    ylab("Country/Region") +
    xlab("Confirmed Cases")
```

```{r}
##Graph No. 3: Graph of number of confirmed deaths for states found in China.
report_03_11_2020 %>% 
  filter (Country.Region == "China") %>% 
  ggplot(aes(x = Deaths,  y = reorder(Province.State, Deaths))) + 
    geom_point() +
    ggtitle("Confirmed deaths for each Chinese province") +
    ylab("Country/Region") +
    xlab("Confirmed Deaths")
```
...
```{r}
##Graph No. 4: Comparison of number of deaths vs number of recovered positive cases of COVID-19 for US
time_series_long_joined_counts %>% 
  group_by(Country_Region, Report_Type, Date) %>% 
  summarise(Counts = sum(Counts)) %>% 
  filter (Country_Region == "US") %>% 
  filter (Report_Type %in% c("Deaths", "Recovered")) %>% 
    ggplot(aes(x = Date,  y = log2(Counts), fill = Report_Type, color = Report_Type)) + 
    geom_point() +
    geom_line() +
    ggtitle("US COVID-19 Deaths versus Recovered")
```
